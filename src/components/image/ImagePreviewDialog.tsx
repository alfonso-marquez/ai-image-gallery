...
import { Sparkles, Tag, Palette, Search, Download } from "lucide-react";
...
export default function ImagePreviewDialog({
  ...
  const providerLabel = useMemo(() => {
    ...
    const openaiOn =
      (process.env.NEXT_PUBLIC_OPENAI_ENABLED || "").toLowerCase() === "true";
    const bedrockOn =
      (process.env.NEXT_PUBLIC_BEDROCK_ENABLED || "").toLowerCase() === "true";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 w-[95vw] sm:max-w-[95vw] md:max-w-[1200px] my-8 max-h-[90vh] overflow-y-auto">
        <DialogHeader className="px-4 pt-4 flex flex-row items-center justify-between">
          <DialogTitle className="truncate">
            {liveImage?.filename ?? "Preview"}
          </DialogTitle>
        </DialogHeader>
        <div className="flex flex-col gap-4 px-4 pb-4">
          {/* Download button row */}
          {liveImage?.original_path && (
            <div className="flex justify-end mb-2">
              <button
                type="button"
                onClick={() =>
                  downloadImage(liveImage.original_path, liveImage.filename)
                }
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md border bg-background hover:bg-muted text-sm font-medium transition"
